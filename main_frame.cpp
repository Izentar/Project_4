...
bool Main_frame::Frame::check_shrink(const int& f_width, const int& f_height)
{
     ...
     {
        Frame *tmp=children[i];
				//if(()tmp->x+tmp->width>f_width)||())
     }
...
bool Main_frame::Frame::check_data(const int& new_x, const int& new_y, const int& f_width, const int& f_height)
{
    if(father==nullptr)
    {
        return check_expand(f_width, f_height);
    }
...
bool Main_frame::Frame::move(const int& OX, const int& OY)  // relative path, it can be negative
{
    ...
    //funkcja do zapisu hierarchicznego


    return false;
}

bool Main_frame::Frame::resize(const int& f_width, const int& f_height)       // absolute path, realtive to (0,0)
{
...
bool Main_frame::resize(const int& f_width, const int& f_height)       // absolute path, realtive to (0,0)
{
